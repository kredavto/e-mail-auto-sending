...
export function EditorPage() {
  ...
  const [guestNoticeOpen, setGuestNoticeOpen] = useState(true);
  const [workspace, setWorkspace] = useState(localStorage.getItem("access_token") ? localStorage.getItem("workspace_id") ?? "" : "");
  const [assistantDraft, setAssistantDraft] = useState<MailTemplate | null>(null);
  ...
  function edit(item: MailTemplate | null) {
    ...
    setAssistantDraft(null); setTemplate(item); setEditorKey(key => key + 1); setDirty(false); setTab("editor"); return true;
  }
  function navigate(target: StudioSection) {
    if (target === "editor" && assistantDraft) { edit(assistantDraft); return; }
    setTab(target);
  }
  useEffect(() => {
    if (!editorKey) return;
    requestAnimationFrame(() => document.getElementById("letter-writing-area")?.scrollIntoView({ block: "start" }));
  }, [editorKey]);
  return <div className="premium-shell">
    <a href="#studio" className="skip-link">Перейти к рабочей области</a>
    <StudioHero onStudio={() => navigate("editor")} />
    ...
    <AccountPanel onAuthenticated={setAuthenticated} workspaceId={workspace} onChange={(id, preserveDraft) => { setWorkspace(id); setAssistantDraft(null); if (preserveDraft) return; setTemplate(null); setContact(null); setEditorKey(key => key + 1); setDirty(false); }} />
    {authenticated && tab === "campaigns" && <div className="workspace-view"><AssistantPanel onDraft={setAssistantDraft} key={`campaigns-${workspace}`} workspaceId={workspace} active schedulingOnly onEdit={edit} /></div>}
    <div className="workspace-view" hidden={tab !== "assistant"}>{authenticated ? <AssistantPanel onDraft={setAssistantDraft} key={`assistant-${workspace}`} workspaceId={workspace} active={tab === "assistant"} onEdit={edit} /> : <section className="panel">ИИ-помощник доступен после входа в личный кабинет. <a href="#account" className="underline">Войти или зарегистрироваться</a></section>}</div>
    ...
    {authenticated && workspace && <AssistantConcierge onDraft={setAssistantDraft} onEdit={edit} key={workspace} workspaceId={workspace} section={tab} onNavigate={target => { setTab(target); if (target !== "editor") document.getElementById("studio")?.scrollIntoView({ behavior: "smooth", block: "start" }); }} />}
