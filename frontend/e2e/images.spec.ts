...
test("upload → quarter-width image → preview → saved template → replace and remove", async ({ page }) => {
  ...
  await page.getByLabel("Файл изображения").setInputFiles({ name: "huge.png", mimeType: "image/png", buffer: Buffer.alloc(5 * 1024 * 1024 + 1) });
  await expect(page.getByRole("alert")).toContainText("5 МБ");
  await page.getByLabel("Файл изображения").setInputFiles({ name: "image.svg", mimeType: "image/svg+xml", buffer: Buffer.from("<svg></svg>") });
  await expect(page.getByRole("alert")).toContainText("JPEG, PNG или WebP");
  await page.getByLabel("Файл изображения").setInputFiles({ name: "photo.png", mimeType: "image/png", buffer: png });
  await page.getByLabel("Описание изображения").fill("Фотография продукта");
  ...
  await image.click();
  await page.getByRole("button", { name: "+ Изображение", exact: true }).click();
  await page.getByLabel("Описание изображения").fill("Новое описание");
  await page.getByLabel("Файл изображения").setInputFiles({ name: "replacement.png", mimeType: "image/png", buffer: png });
  await page.getByRole("button", { name: "Обновить изображение" }).click();
