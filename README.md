...
статус, отписку и JSON `data` со всеми исходными полями контакта. При переименовании
базы меняется имя её таблицы; состав и обновления отражаются при очередной синхронизации.

Основная PostgreSQL приложения остаётся источником данных для кампаний и глобальных
отписок. Supabase — отдельное синхронизируемое хранилище баз, а не замена авторизации или
очереди. `app.modules.contact_storage.tasks.sync_contacts` проверяет изменения каждые
5 минут; неизменившиеся базы не копируются повторно. Копирование идёт порциями по 500
