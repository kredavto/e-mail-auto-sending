...
ИИ-помощник: создание/улучшение писем, личная история, подсказки, подготовка рассылок
и подтверждаемое управление расписанием. [Пошаговая инструкция](docs/ai-assistant.md).
...
8. Для фотографии нажмите **+ Фото / видео**, выберите JPEG / PNG / WebP до 5 МБ
   (до 20 мегапикселей, без анимации), при желании заполните описание и нажмите
   **Загрузить и вставить**. Изображение занимает 25% ширины текстовой области
   с сохранением пропорций; HTML-письмо ограничивает его ширину 144 px (четверть
   стандартной области 576 px). Оно сохраняется в шаблоне и HTML-экспорте.
   Для замены/изменения описания/удаления выделите фото и снова нажмите
   **+ Фото / видео**. После изменений нажмите **Сохранить шаблон**.
9. В том же окне можно загрузить **MP4 до 20 МБ, 60 секунд и 4K**. Сервер проверяет
   контейнер и длительность, преобразует видео в H.264/AAC (до 720×720 с сохранением
   пропорций, до 30 кадров/с) и создаёт кадр-превью с символом ▶. В окне загрузки
   доступен видеоплеер; в письме — кликабельное превью в ¼ ширины текста и подпись.
   Нажатие открывает MP4 в браузере. Встроенное видео в HTML-письмах ненадёжно,
   поэтому `<video>` в отправляемый шаблон не включается. Файл можно заменить или
   удалить из шаблона; старые фотографии и шаблоны остаются совместимыми.

Видео API: авторизованный `POST /api/v1/files/videos`, публичные `GET`/`HEAD`
`/api/v1/files/videos/{random-id}.mp4` с поддержкой Range/206 для перемотки.
S3 namespace `email-videos/` не раскрывает контактные импорты. Видео и превью
доступны по неугадываемым ссылкам и сохраняются для уже отправленных писем.
Docker-образ включает FFmpeg/ffprobe; при запуске Python без Docker установите их
в PATH. Обработка ограничена двумя одновременными задачами на процесс и таймаутом;
произвольные внешние URL или файловые ссылки внутри MP4 не загружаются.
Проверки: `pytest tests/test_email_videos.py`; браузерные тесты используют
синтетический двухсекундный MP4 с тестовой таблицей и тоном, а не клиентские файлы.

Изображения загружаются авторизованными участниками workspace через
`POST /api/v1/files/images`; сервер проверяет содержимое, убирает EXIF/GPS и
перекодирует в PNG до 1200×1200 px. S3 namespace `email-images/` отделён от
контактных импортов. `GET /api/v1/files/images/{random-id}.png` публичен по
неугадываемой ссылке, чтобы почтовые клиенты получателей могли загрузить фото.
Не загружайте конфиденциальные изображения. Удаление блока из редактора не
удаляет объект: он может использоваться в других версиях и уже отправленных письмах.
`PUBLIC_BASE_URL` должен указывать на доступный получателям HTTPS backend, включая
префикс `/mailer`, если он используется прокси. Некоторые почтовые клиенты
блокируют внешние картинки до разрешения получателя; в этом случае видно описание.
Проверки изображений: `pytest tests/test_email_images.py` и `pnpm --dir frontend test:e2e`.
...
## Массовые рассылки по расписанию

В студии открыт отдельный раздел **«Рассылки по расписанию»**. Создайте новую рассылку,
выберите именованную базу или поиск, нажмите «Выбрать всю выборку» (не более 50 000
получателей), задайте шаблоны, отправителя и дату/время МСК. Режим «Запланировать
отправку» после подтверждения параметров сохраняет кампанию как `scheduled`; режим
«Сохранить черновик» ничего не отправляет. Запланированные кампании проверяются Celery
Beat раз в минуту; браузер можно закрыть. Первый шаг не исполняется раньше заданного
времени. Письма отправляются постепенно с учётом SMTP/SES, часовых и дневных лимитов.

Шаг продвигается после успешного выполнения worker. При временном лимите адрес остаётся
в кампании с будущим временем попытки; отписки, ответы и возвраты по-прежнему учитываются.
Запланированную кампанию можно приостановить через управление расписанием. Для изменения
времени сначала поставьте её на паузу (перенос доступен до постановки первого шага в очередь).

## Именованные базы в Supabase

Проект `premium-b2b-mailer` (`zampkbmunemfxfegaztc`) хранит именованные контактные базы
в приватной схеме `mailer_contacts`. Для каждой базы создаётся отдельная таблица:
читаемое название + UUID базы (суффикс исключает коллизии, полное название хранится
в `mailer_contacts.bases`). Таблицы содержат email, ФИО, компанию, должность, телефон,
статус, отписку и JSON `data` со всеми исходными полями контакта.
В Supabase сохраняется снимок базы на момент завершения загрузки.

Основная PostgreSQL приложения остаётся источником данных для кампаний и глобальных
отписок. Supabase хранит копии загруженных баз. После успешной отправки всех порций
файла интерфейс вызывает `POST /contacts/lists/{id}/import-complete`. Сервер ставит
задачу только для этой базы и рабочего пространства. Периодической синхронизации нет.
При временном сбое задача повторяется до пяти раз; неизменившийся снимок не копируется
повторно. Копирование идёт порциями по 500
контактов из согласованного снимка. Старые записи удаляются из копии только после
проверки количества успешно полученных строк. При сбое следующая попытка повторяет
синхронизацию; исходные контакты остаются в PostgreSQL приложения.

Серверные переменные (не передавать во frontend):

- `SUPABASE_CONTACT_STORAGE_URL`: URL Edge Function `contact-storage`;
- `SUPABASE_CONTACT_STORAGE_TOKEN`: случайный секрет серверного подключения.

Edge Function проверяет SHA-256 токена до обращения к БД. В исходниках находится только
проверочный хеш; сам токен хранится в серверном `.env`. SQL выполняется от отдельной
роли `mailer_contacts_writer` в приватной схеме, на таблицах включены RLS и ограничение
по рабочему пространству. `anon` и `authenticated` не имеют доступа к этим таблицам.
Схема создаётся из `infrastructure/supabase/contact-storage.sql`; код функции находится
в `supabase/functions/contact-storage`. В разделе контактов показываются имя таблицы
и время последней успешной синхронизации.
